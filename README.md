...
##### 7. `notebooks/copilot_practice.ipynb`
- This notebook serves as a sandbox for workshop participants to gain hands-on experience with GitHub Copilot. It contains step-by-step tasks related to cleaning data, summarizing metrics, or generating charts. The idea is to build familiarity with using Copilot by typing descriptive comments and seeing the code auto-complete. Each task aligns with one part of the LRTP process, so participants can understand not just how to use Copilot, but also where it fits into a real-world transportation planning workflow.
